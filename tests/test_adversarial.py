...
async def test_generate_from_template_valid(generator):
    ...
    generator.generate_responses = AsyncMock(return_value={"data": {"response": ["Alice is a teacher", "Bob is a doctor"]}})
...
async def test_generate_from_template_invalid_style(generator):
    with pytest.raises(ValueError):
        await generator._generate_from_template({"text": ["test"]}, ["invalid"], count=1)
...
def test_format_result_structure(generator):
    ...
    result = generator._format_result(dataset, prompt_templates, keys=["text", "group", "group_category"])
...
def test_read_counterfactual_data_valid():
    mock_json = json.dumps([
        {"text": "X is Y", "group": "Homosexuals", "group_category": "Sexual orientation"},
        {"text": "Z is W", "group": "Able-bodied people", "group_category": "Disability"}
    ])
    with patch("builtins.open", mock_open(read_data=mock_json)), \
         patch("pkgutil.resolve_name", return_value=MagicMock(__file__=os.path.join("langfair", "__init__.py"))):
...
def test_read_toxicity_data_toxic():
    mock_lines = [
        json.dumps({"prompt": {"text": "Why are cats evil?", "toxicity": 0.9}, "challenging": True}),
        json.dumps({"prompt": {"text": "Why do people lie?", "toxicity": 0.8}, "challenging": False}),
    ]
...
def test_read_toxicity_data_nontoxic():
    mock_lines = [
        json.dumps({"prompt": {"text": "Why are cats cute?", "toxicity": 0.05}, "challenging": False}),
        json.dumps({"prompt": {"text": "Why do people smile?", "toxicity": 0.2}, "challenging": False}),
    ]
    with patch("builtins.open", mock_open(read_data="\n".join(mock_lines))), \
         patch("pkgutil.resolve_name", return_value=MagicMock(__file__=os.path.join("langfair", "__init__.py"))), \
